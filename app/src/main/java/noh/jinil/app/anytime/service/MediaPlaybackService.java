...
import noh.jinil.app.anytime.widget.VisualizerWidgetProvider;
...
public class MediaPlaybackService extends Service {
    ...
    public static final String PLAYSTATE_CHANGED = VisualizerManager.PACKAGE_NAME+".playstatechanged";
    public static final String META_CHANGED = VisualizerManager.PACKAGE_NAME+".metachanged";
    public static final String SHUFFLE_CHANGED = VisualizerManager.PACKAGE_NAME+".shuffle";
    public static final String REPEAT_CHANGED = VisualizerManager.PACKAGE_NAME+".repeat";
    public static final String QUEUE_CHANGED = VisualizerManager.PACKAGE_NAME+".queuechanged";
    public static final String INFO_CHANGED = VisualizerManager.PACKAGE_NAME+".infochanged";
    ...
    private static final int OPEN_AND_NEXT = 9;
    private static final int CHECK_VISUALIZER = 10;
    ...
    private Handler mMediaplayerHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                // added by rollingstone
                case OPEN_AND_NEXT:
                	mCurrentVolume -= .05f;
                    if (mCurrentVolume > .2f) {
                        mMediaplayerHandler.sendEmptyMessageDelayed(OPEN_AND_NEXT, 15);
                        mPlayer.setVolume(mCurrentVolume);
                    } else {
                        stop(false);
                        openCurrentAndNext();
                        play();
                        mMediaplayerHandler.sendEmptyMessageDelayed(FADEUP, 20);
                        notifyChange(INFO_CHANGED);
                    }
                	break;
                	 
                case CHECK_VISUALIZER:
                	Log.w(TAG, "->CHECK_VISUALIZER");
                	if(!VisualizerManager.getInstance().isVisualizerActive() && !PreferenceUtils.loadBooleanValue(getApplicationContext(), PreferenceUtils.KEY_MIC_USE, false)) {
//                		if(PreferenceUtils.loadBooleanValue(getApplicationContext(), PreferenceUtils.KEY_RC_AUTHORIZED, false)) {
//                			Toast.makeText(getApplicationContext(), R.string.noti_global_visualizer_setting, Toast.LENGTH_LONG).show();
//                			VisualizerManager.getInstance().setSessionID(0);
//                		}
//                		else {
//                			Toast.makeText(getApplicationContext(), R.string.noti_local_visualizer_setting, Toast.LENGTH_LONG).show();
//                			VisualizerManager.getInstance().setSessionID(mPlayer.getAudioSessionId());
//                		}
    ...
    private BroadcastReceiver mIntentReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (CMDNEXT.equals(cmd) || NEXT_ACTION.equals(action)) {
                ...
                mPausedByTransientLossOfFocus = false;
                seek(0);
            } else if (VisualizerWidgetProvider.CMDAPPWIDGETUPDATE.equals(cmd)) {
            	...
            	if(mFloatingView.isShowing()) {
            		mFloatingView.hide();
            		mFloatingView.showNotification();
            	} else {
            		mFloatingView.show();
            		mFloatingView.showNotification();
            	}
            }
            else if(VisualizerAnytime.TOGGLE_MIC_USE.equals(action)) {
                boolean MicUse = PreferenceUtils.loadBooleanValue(getApplicationContext(), PreferenceUtils.KEY_MIC_USE, false);
                if (MicUse) {
                    mFloatingView.setupSession();
                    MicUse = false;
                } else {
                    mFloatingView.setupMic();
                    MicUse = true;
                }
                PreferenceUtils.saveBooleanValue(getApplicationContext(), PreferenceUtils.KEY_MIC_USE, MicUse);
                mFloatingView.showNotification();
            }
            else if(VisualizerAnytime.TOGGLE_VIEW_RATIO.equals(action)) {
            	mFloatingView.plusViewRatio();
            }
        }
    };
    ...
    public void onCreate() {
        ...
        mFloatingView = new FloatingView(this, getAudioSessionId());
        notifyChange(INFO_CHANGED);
        ...
        commandFilter.addAction(VisualizerAnytime.TOGGLE_MIC_USE);
        commandFilter.addAction(VisualizerAnytime.CLOSE_FLOATING_VIEW);
        commandFilter.addAction(VisualizerAnytime.TOGGLE_FLOATING_VIEW);
        commandFilter.addAction(VisualizerAnytime.TOGGLE_VIEW_RATIO);
    ...
    public void onDestroy() {
        ...
        // hide visualizer
        mFloatingView.hide();
        mFloatingView.hideNotification();
        stopForeground(true);
    ...
    private void saveQueue(boolean full) {
        ...
        if (full) {
            ...
            ed.putString("queue_version1", q.toString());
    ...
    private void reloadQueue() {
        ...
        if (id == mCardId) {
            ...
            q = mPreferences.getString("queue_version1", "");
        }
        ...
        if (qlen > 1) {
            ...
            // Fixed by rollingstone begin
            if (shufmode == SHUFFLE_AUTO) {
            	shufmode = SHUFFLE_NONE;
            }
            // Fixed by rollingstone end
                ...
                if (qlen > 1) {
                    ...
                    for (int i = 0; i < qlen; i++) {
                        ...
                        if (c == ';') {
                            ...
                            Log.e(TAG, "mHistory.add reloadQueue");
                            mHistory.add(n);
                            n = 0;
                            shift = 0;
                        } else {
...
//        return START_STICKY;
    ...
    private void notifyChange(String what) {
        ...
        if (what.equals(PLAYSTATE_CHANGED)) {
        	if(mFloatingView.isNowNotiShow) {
        		mFloatingView.showNotification();
        	}
        }
        else if (what.equals(INFO_CHANGED)) {
            if(mFloatingView.isNowNotiShow) {
        		mFloatingView.showNotification();
            }
        }
     ...
     */
    public int enqueue(long [] list, int action) {
        synchronized(this) {
    ...
    public void play() {
        ...
        if (mPlayer.isInitialized()) {
            ...
            // Added by MrJino begin
            mMediaplayerHandler.removeMessages(CHECK_VISUALIZER);
            mMediaplayerHandler.sendEmptyMessageDelayed(CHECK_VISUALIZER, 3000);
            // Added by MrJino End
    ...
    public void prev() {
        synchronized (this) {
            if (mShuffleMode == SHUFFLE_NORMAL) {
                ...
                Log.e(TAG, "histsize:"+histsize);
                if (histsize == 0) {
                    // prev is a no-op
                    return;
                }
                for(int i=0;i<histsize;i++) {
                	Log.e(TAG, i+".item:"+mHistory.get(i));
                }
            ...
            saveBookmarkIfNeeded();
            mMediaplayerHandler.sendEmptyMessage(OPEN_AND_NEXT);
//            stop(false);
//            openCurrentAndNext();
//            play();
            notifyChange(META_CHANGED);
    ...
    private int getNextPosition(boolean force) {
        if (mRepeatMode == REPEAT_CURRENT) {
            ...
            // reasonable size
            if (mPlayPos >= 0) {
            	Log.e(TAG, "mHistory.add getNextPosition");
    ...
    public void gotoNext(boolean force) {
        synchronized (this) {
            ...
            saveBookmarkIfNeeded();
            mPlayPos = pos;
            mMediaplayerHandler.sendEmptyMessageDelayed(OPEN_AND_NEXT, 10);
//          stop(false);
//          openCurrentAndNext();
//          play();
            notifyChange(META_CHANGED);
...
//        stopForeground(true);
    ...
    private void doAutoShuffleUpdate() {
        ...
        for (int i = 0; i < to_add; i++) {
            ...
            Log.e(TAG, "mHistory.add doAutoShuffleUpdate");
            mHistory.add(idx);
            if (mHistory.size() > MAX_HISTORY_SIZE) {
                mHistory.remove(0);
            }
    ...
    public void playAutoShuffleList() {
    	if (makeAutoShuffleList()) {
            mPlayListLen = 0;
            open(mAutoShuffleList, 0);
            // doAutoShuffleUpdate();
            
            if(mPlayListLen == 0) {
            	Toast.makeText(getApplicationContext(), R.string.toast_nocontent_toplay, Toast.LENGTH_LONG).show();
            } else {
            	openCurrentAndNext();
                play();
                notifyChange(META_CHANGED);
    ...
    public void setShuffleMode(int shufflemode) {
        synchronized(this) {
            ...
            notifyAttributeValues(PLAYERSETTINGS_RESPONSE, supportedAttributes, NOTIFY_ATTRIBUTE_VALUES);
...
//        }
    ...
    static class ServiceStub extends IMediaPlaybackService.Stub {
        ...
        public int enqueue(long [] list , int action) {
            return mService.get().enqueue(list, action);
        }
        public long [] getQueue() {
	    ...
	    public void showFloatingView(boolean miniview) {
	    	if(mService.get().mFloatingView != null) {
	    		if(miniview) {
	    			mService.get().mFloatingView.show();
	    		}
	    		mService.get().mFloatingView.showNotification();	    		
	    	}
		}
	    public void hideFloatingView() {
	    	if(mService.get().mFloatingView != null) {
	    		mService.get().mFloatingView.hide();
	    		mService.get().mFloatingView.hideNotification();
	    	}
		}
		public void closeFloatingView() throws RemoteException {
			if(mService.get().mFloatingView != null) {
	    		mService.get().mFloatingView.close();
	    	}
		}
		public void playAutoShuffleList() throws RemoteException {
	    	mService.get().playAutoShuffleList();
		}		
    }
    
    public boolean isFloatingView() {
    	return mFloatingView.isShowing();
    }
    ...
    class FloatingView {
    	...
    	FloatingView(Context context, int sessionID) {
    		mContext = context;
    		
    		mAnyTime = new VisualizerAnytime(mContext, sessionID);
    		initNotification();
    	}
    	
		void show() {
			mAnyTime.showFloatingWindow(mContext);
			mAppWidgetProvider.performUpdate(MediaPlaybackService.this, null);
		}
		
		void hide() {
			mAnyTime.hideFloatingWindow(mContext);			
			mAppWidgetProvider.performUpdate(MediaPlaybackService.this, null);
		}
		
		void update() {
			mAnyTime.updateFloatingWindow(mContext);
		}
		...
		void close() {
			...
			mDelayedStopHandler.removeCallbacksAndMessages(null);
	        Message msg = mDelayedStopHandler.obtainMessage();
	        mDelayedStopHandler.sendMessageDelayed(msg, 1000);
		}
		
		boolean isShowing() {
			return mAnyTime.isFloatingView();
		}
        ...
        public void setupSession() {
            mAnyTime.setupSession();
        }
		
		void plusViewRatio() {
			mAnyTime.plusViewRatio();
		}		
		
		void initNotification() {
//			Intent i = new Intent(mContext, MainActivity.class);
//	        i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
//			PendingIntent pi = PendingIntent.getActivity(mContext.getApplicationContext(), 0, i, 0);
			
			Notification.Builder NotiBuilder = new Notification.Builder(mContext);		
//			NotiBuilder.setContentIntent(pi);
			NotiBuilder.setSmallIcon(R.drawable.statusbar);
			
			if(android.os.Build.VERSION.SDK_INT >= 16) {
				mNotification = NotiBuilder.build();
			} else {
				mNotification = NotiBuilder.getNotification();
			}
		...
		void showNotification() {
			boolean hideNoti = PreferenceUtils.loadBooleanValue(getApplicationContext(), PreferenceUtils.KEY_NOTI_HIDE, false);
			if(hideNoti)
				return;
			
			isNowNotiShow = true;
			
			RemoteViews v = new RemoteViews(mContext.getPackageName(), R.layout.notification);
					
			Intent i;
			PendingIntent pi;
			
			if(isShowing()) {
				v.setImageViewResource(R.id.noti_visualizer_onoff, R.drawable.noti_visualizer_on);
			} else {
				v.setImageViewResource(R.id.noti_visualizer_onoff, R.drawable.noti_visualizer_off);
			}
			
//			boolean confirm = PreferenceUtils.loadBooleanValue(mContext, PreferenceUtils.KEY_NOTIFICATION_HELP, false);
//			if(confirm) {
//				v.setViewVisibility(R.id.notification_help_guide, View.GONE);
//			} else {
//				v.setViewVisibility(R.id.notification_help_guide, View.VISIBLE);
//			}
			
			if(isPlaying()) {
				v.setImageViewResource(R.id.noti_button_control, R.drawable.music_button_pause);
			} else {
				v.setImageViewResource(R.id.noti_button_control, R.drawable.music_button_play);
			}

			if (getAudioId() < 0) {
	            // streaming
	            v.setTextViewText(R.id.track_info, getString(R.string.app_title));
	        } else {
	            v.setTextViewText(R.id.track_info, getTrackName()+" - "+getArtistName());
	        }

            if(PreferenceUtils.loadBooleanValue(mContext, PreferenceUtils.KEY_MIC_USE, false)) {
                v.setImageViewResource(R.id.noti_button_usemic, R.drawable.microphone_on);
                v.setTextViewText(R.id.track_info, getString(R.string.noti_use_mic));
            } else {
                v.setImageViewResource(R.id.noti_button_usemic, R.drawable.microphone_off);
            }
					    
		    i = new Intent(VisualizerAnytime.TOGGLE_FLOATING_VIEW);
		    pi = PendingIntent.getBroadcast(mContext, 0, i, 0);
			v.setOnClickPendingIntent(R.id.noti_visualizer_onoff, pi);
			
			i = new Intent(PREVIOUS_ACTION);
			pi = PendingIntent.getBroadcast(mContext, 0, i, 0);
		    v.setOnClickPendingIntent(R.id.noti_button_prev, pi);
		    
		    i = new Intent(NEXT_ACTION);
			pi = PendingIntent.getBroadcast(mContext, 0, i, 0);
		    v.setOnClickPendingIntent(R.id.noti_button_next, pi);
		    
		    i = new Intent(TOGGLEPAUSE_ACTION);
			pi = PendingIntent.getBroadcast(mContext, 0, i, 0);
		    v.setOnClickPendingIntent(R.id.noti_button_control, pi);

            i = new Intent(VisualizerAnytime.TOGGLE_MIC_USE);
            pi = PendingIntent.getBroadcast(mContext, 0, i, 0);
            v.setOnClickPendingIntent(R.id.noti_button_usemic, pi);
		    
		    i = new Intent(VisualizerAnytime.CLOSE_FLOATING_VIEW);
		    pi = PendingIntent.getBroadcast(mContext, 0, i, 0);
		    v.setOnClickPendingIntent(R.id.noti_button_close, pi);

            i = new Intent(mContext, MainActivity.class);
	        i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
            pi = PendingIntent.getActivity(mContext.getApplicationContext(), 0, i, 0);
